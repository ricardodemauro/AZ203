...
namespace ConsoleAppTableStorage
{
    public class TableManager
    {
        ...
        public TableManager(string tableName)
            : this(tableName, "DefaultEndpointsProtocol=https;AccountName=myamazingstorageaccount;AccountKey=SVFWlIHOvYdro4ssRo5a1F50L65vumQM+UqhoZAr0zEHRvlzZCNpNEPRxO0BQJYgbjfiWkBZXOAwZ3gXSDaS0Q==;EndpointSuffix=core.windows.net")
        {
        ...
        public async Task<bool> Add(EmployeeSessionEntity entity, CancellationToken cancellationToken = default)
        {
            TableOperation insertOp = TableOperation.Insert(entity);

            var tableResult = await _table.ExecuteAsync(insertOp);
            return (int)HttpStatusCode.OK == tableResult.HttpStatusCode;
        }
